...
public class GameController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 弹窗面板比取景框高，先统一缩放一次，保证成功/失败弹窗完整可见。
        DialogFitter.Fit(gameOverWin);
        DialogFitter.Fit(pauseWin);
    }
    void GameTimer()
    {
        spendTime += 1;
    ...
    IEnumerator AutoCreateEnemy()
    {
        ...
        while (vocList.Count > 0)
        {
            if (enemyList.Count < currLv.maxEnemyCount)
            {
                ...
                // 单词窗口挂在小怪头顶。换皮后小怪高度可能变了，优先用换皮后的实际高度。
                float enemyHeight = enemyObj.GetComponent<SpriteRenderer>().bounds.size.y;
                var skinCtrl = enemyCtrl.ApplyCharacterSkin();
                if (skinCtrl != null && skinCtrl.WorldHeight > 0f)
                    enemyHeight = skinCtrl.WorldHeight;
                vocWinObj.transform.localPosition = new Vector3(0, enemyHeight * 0.5f, 0);

                // 单词框原来又扁又小，英文和中文都看不清 —— 拉高并放大字号。
                VocWinLayout.Apply(vocWinObj);
